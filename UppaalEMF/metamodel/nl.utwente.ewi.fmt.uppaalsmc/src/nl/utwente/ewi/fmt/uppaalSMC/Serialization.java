...
public class Serialization extends UppaalSerialization {
	...
	@Override
	public Point planarPosition(EObject obj, String featureName)
	{
		if (obj == null)
			return null;
	...
	public CharSequence location(Location l)
	{
		...
		if (l instanceof ExponentialLocation) {
			Point namePoint = planarPosition(l, "namePosition");
			b.append("<name");
			b.append(position(namePoint));
			b.append(">").append(name(l)).append("</name>");
			if (l.getInvariant() != null) {
				Point invariantPoint = planarPosition(l.getInvariant(), "position");
				b.append("<label kind=\"invariant\"");
				b.append(position(invariantPoint));
				b.append(">").append(expression(l.getInvariant().getExpression())).append("</label>");
			}
	...
	public CharSequence edge(Edge e)
	{
		...
		if (e.getSelection() != null && !e.getSelection().isEmpty()) {
			Point selectPoint = planarPosition(e.getSelection().get(0), "position");
			ret.append("<label kind=\"select\"");
			ret.append(position(selectPoint));
			ret.append(">");
		...
		first = true;
		if (e.getUpdate() != null && e.getUpdate().getExpression() != null && !e.getUpdate().getExpression().isEmpty()) {
			Point assignmentPoint = planarPosition(e.getUpdate(), "position");
			ret.append("<label kind=\"assignment\"");
			ret.append(position(assignmentPoint));
			ret.append(">");
			for (Expression s : e.getUpdate().getExpression()) {
				if (!first) {
					ret.append(", ");
				}
				first = false;
		...
		Point probabilityPoint = ((ChanceEdge)e).getProbabilityPosition();
		ret.append("<label kind=\"probability\"");
		ret.append(position(probabilityPoint));
		ret.append(">");
