// Duplication collector for ETL transformations - ALL files (including uppaal code syntax library)
// Outputs: compare-results/duplicationDirectory_all_{N}lines.json
const fs = require('fs');
    ...
    fs.readdir(dir, { withFileTypes: true }, (err, files) => {
        ...
        files.forEach((file) => {
            ...
            if (file.isDirectory()) {
                ...
                if (path.extname(file.name).toLowerCase() === EXTENSION) fileCount++;
                if (!--pending) callback(null, fileCount);
            }
        });
    });
...
function readFilesRecursively(dir) {
    ...
    myPromise.then(() => {
        ...
        const jsonFilePath = path.join(RESULTS_DIR, `duplicationDirectory_all_${linesAmount}lines.json`);
...
countEtlFilesRecursively(TARGET_DIR, (err, localCount) => {
    ...
    console.log("Total ETL files to scan: " + fileCount);
