...
class UppaalSerialization {
	...
	def id(Location it) {
		val templateName = if (parentTemplate == null) "" else parentTemplate.name
		if (name != null && name.length > 0) {
			return name + "_" + templateName
		}
		val idx = if (parentTemplate == null || parentTemplate.location == null) 0 else parentTemplate.location.indexOf(it)
		return "loc" + idx + "_" + templateName
	}
	...
	<location id="«id(it)»" x="«if (position == null) "" else position.x»" y="«if (position == null) "" else position.y»" «IF colorToString(it) != null»color="«colorToString(it)»"«ENDIF»>
		<name«pointAttributes(planarPosition(it, "namePosition"))»>«name(it)»</name>
		«IF !(invariant == null)»<label kind="invariant"«pointAttributes(planarPosition(invariant, "position"))»>«expression(invariant.expression)»</label>«ENDIF»
	...
	«IF !(selection == null) && !selection.empty»<label kind="select"«pointAttributes(planarPosition(firstSelection, "position"))»>«FOR i : selection ?:emptyList SEPARATOR ', '»«selection(i)»«ENDFOR»</label>«ENDIF»
	«IF !(guard == null)»<label kind="guard"«pointAttributes(planarPosition(guard, "position"))»>«expression(guard.expression)»</label>«ENDIF»
	«IF !(synchronization == null)»<label kind="synchronisation"«pointAttributes(planarPosition(synchronization, "position"))»>«synchronization(synchronization)»</label>«ENDIF»
	«IF !(update == null)»<label kind="assignment"«pointAttributes(planarPosition(update, "position"))»>«FOR i : update.expression ?:emptyList SEPARATOR ","»
	«expression(i)»
	«ENDFOR»</label>«ENDIF»
	...
	def pointAttributes(Point it) '''«IF it != null» x="«x»" y="«y»"«ENDIF»'''

	def planarPosition(EObject it, String featureName) {
		if (it == null) {
			return null;
		}
		val feature = it.eClass.getEStructuralFeature(featureName)
		if (feature == null) {
			return null;
		}
		val value = it.eGet(feature)
